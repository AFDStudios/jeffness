...
echo '<section class="' . $className . 'content-grid-wrapper" id="' . $id . '"' . (get_field('background_color')?' style="background-color: '.get_field('background_color').'"':'') . '>';
	if ( get_field('headline') ) { echo '<h2 class="content-grid-headline"' . ( !empty( get_field('headline_color') )?' style="color:' . get_field('headline_color').';"':'') . '>' . get_field('headline') . '</h2>'; }
	if ( get_field('intro_text') ) { echo '<div class="content-grid-intro">' . get_field('intro_text') . '</div>'; }
	if ( have_rows( 'grid_items' ) ) :
		echo '<div class="content-grid-items-wrapper constrained-width-narrow content-grid-columns-' . get_field('columns') . '">';
